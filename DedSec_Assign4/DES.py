...
def encrypt(plain_text, sub_keys):
  ...
  out = func(right,sub_keys[2])
  temp = int(out, base=2) ^ int(left , base=2)
  left = '{0:032b}'.format(temp)
  final = last_perm(left+right)            #Inverse permutation after 16 rounds
  cipher = ""
  for i in range(16):
    cipher += '{0:x}'.format(int(final[4*i:4*i+4], base=2))
  return cipher
